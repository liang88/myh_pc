...
export default {
  ...
  methods: {
    loadsay () {
      ...
      this.$http.post(myHost+ 'myh_web/selectOneLayImg', typeinfo).then((response) => {
    ...
    pinglun () {
      var plinfo = {
        houseId: parseInt(this.$route.params.id),
        page: {
          pageNum: this.pagethis,
          pageSize: 10
        }
      }
      this.$http.post(myHost+ 'myh_web/selectOneLayImg', plinfo).then((response) => {
        // console.log(response)
        var data = response.data
        data = data.resultBean
        data = data.object
        var pinglundate = data.commList
